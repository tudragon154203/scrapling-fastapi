"""Metadata and cleanup helpers for Chromium user-data profiles."""
...
class ChromiumProfileHousekeeping:
    """Handle profile metadata access and clone cleanup heuristics."""
    ...
    def cleanup_old_clones(
        self,
        max_age_hours: int = 24,
        max_count: int = 50,
    ) -> Dict[str, int]:
        if (  # pragma: no cover - trivially exercised through manager tests
            not self._enabled
            or self._profile_manager is None
            or not self._path_manager.clones_dir.exists()
        ):
        ...
        try:
            ...
            for clone_stat in to_remove:
                ...
                try:
                    ...
                    logger.warning(
                        "Failed to remove clone directory %s: %s",
                        path,
                        exc,
                    )
                    error_count += 1
