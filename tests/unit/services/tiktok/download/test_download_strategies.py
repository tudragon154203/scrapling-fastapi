...
from app.services.tiktok.download.strategies.factory import (
...
class TestCamoufoxDownloadStrategy:
    ...
    def test_resolve_video_url_success(
        self, strategy: CamoufoxDownloadStrategy
    ) -> None:
        ...
        with patch.object(strategy, "_build_camoufox_fetch_kwargs") as mock_build:
            ...
            result = strategy.resolve_video_url(
                "https://www.tiktok.com/@test/video/123"
            )

            assert result == "https://example.com/video.mp4"

    def test_resolve_video_url_fallback_to_html(
        self, strategy: CamoufoxDownloadStrategy
    ) -> None:
    ...
    def test_resolve_video_url_failure(
        self, strategy: CamoufoxDownloadStrategy
    ) -> None:
    ...
    def test_filter_media_links(self, strategy: CamoufoxDownloadStrategy) -> None:
        ...
        assert (
            "https://tikvid.io/info" in filtered
        )  # Kept because doesn't start with tikvid.io/vi
        assert "https://example.com/video.mp4?mime_type=video_mp4" in filtered
        assert "https://example.com/image.jpg" in filtered
        assert (
            "https://tikvid.io/vi/info" not in filtered
        )  # Filtered out - starts with TIKVID_BASE and no mp4
...
class TestChromiumDownloadStrategy:
    ...
    def strategy(self, mock_settings: MagicMock) -> ChromiumDownloadStrategy:
        ...
        strategy = ChromiumDownloadStrategy(mock_settings)
        # Prevent real user data interactions in unit tests
        strategy.user_data_context_provider = MagicMock()
        strategy.user_data_context_provider.acquire_read_context.return_value = (
            ChromiumUserDataContext(effective_dir=None, cleanup=None)
        )
        return strategy
    ...
    def test_resolve_video_url_success(
        self, strategy: ChromiumDownloadStrategy
    ) -> None:
        ...
        with patch.object(strategy, "_build_chromium_fetch_kwargs") as mock_build:
            ...
            assert (
                call_args[0][0] == "https://www.tiktok.com/@test/video/456"
            )  # tiktok_url
            assert call_args[0][1] is None  # quality_hint

    def test_chromium_strategy_enforces_headful_fetch_kwargs(
        self, strategy: ChromiumDownloadStrategy
    ) -> None:
    ...
    def test_chromium_strategy_build_fetch_kwargs_headful_enforcement(
        self, strategy: ChromiumDownloadStrategy
    ) -> None:
    ...
    def test_chromium_strategy_injects_user_data_dir_when_enabled(
        self, strategy: ChromiumDownloadStrategy
    ) -> None:
        ...
        strategy.user_data_context_provider.acquire_read_context.return_value = (
            ChromiumUserDataContext(
                effective_dir="/path/to/user/data",
                cleanup=cleanup,
            )
        )

        with patch(
            "app.services.tiktok.download.strategies.chromium.fetch_method_supports_argument",
        ...
        assert result["fetch_kwargs"]["user_data_dir"] == "/path/to/user/data"
        assert (
            result["fetch_kwargs"]["additional_args"]["user_data_dir"]
            == "/path/to/user/data"
        )
        assert result["user_data_cleanup"] is cleanup
        strategy.user_data_context_provider.acquire_read_context.assert_called_once()

    def test_chromium_strategy_headless_when_force_headful_false(
        self, strategy: ChromiumDownloadStrategy
    ) -> None:
        ...
        with patch.object(strategy, "_build_chromium_fetch_kwargs") as mock_build:
            ...
            result = strategy.resolve_video_url(
                "https://www.tiktok.com/@test/video/456", force_headful=False
            )
            ...
            mock_build.assert_called_once_with(
                "https://www.tiktok.com/@test/video/456", None, False
            )

    def test_chromium_strategy_headful_when_force_headful_true(
        self, strategy: ChromiumDownloadStrategy
    ) -> None:
        ...
        with patch.object(strategy, "_build_chromium_fetch_kwargs") as mock_build:
            ...
            result = strategy.resolve_video_url(
                "https://www.tiktok.com/@test/video/456", force_headful=True
            )
            ...
            mock_build.assert_called_once_with(
                "https://www.tiktok.com/@test/video/456", None, True
            )

    def test_chromium_strategy_parity_features_in_headless(
        self, strategy: ChromiumDownloadStrategy
    ) -> None:
        ...
        assert (
            result["fetch_kwargs"]["network_idle"] is True
        )  # Headless uses network idle
        assert result["fetch_kwargs"]["wait"] == 5000  # Headless uses longer wait
        assert (
            result["fetch_kwargs"]["timeout"] == 120000
        )  # Headless uses longer timeout
    ...
    def test_chromium_strategy_headful_mode_unchanged(
        self, strategy: ChromiumDownloadStrategy
    ) -> None:
        ...
        assert (
            result["fetch_kwargs"]["network_idle"] is False
        )  # Headful doesn't use network idle
        assert result["fetch_kwargs"]["wait"] == 3000  # Headful uses shorter wait
        assert (
            result["fetch_kwargs"]["timeout"] == 90000
        )  # Headful uses standard timeout

    def test_chromium_strategy_user_data_in_both_modes(
        self, strategy: ChromiumDownloadStrategy
    ) -> None:
        """Test that user data management works in both headless and headful modes."""
        cleanup_headful = MagicMock()
        cleanup_headless = MagicMock()
        strategy.user_data_context_provider.acquire_read_context.side_effect = [
            ChromiumUserDataContext(
                effective_dir="/path/to/headful", cleanup=cleanup_headful
            ),
            ChromiumUserDataContext(
                effective_dir="/path/to/headless", cleanup=cleanup_headless
            ),
        ]
        ...
        assert result_headful["effective_user_data_dir"] == "/path/to/headful"
        assert result_headful["user_data_cleanup"] is cleanup_headful
        assert result_headless["effective_user_data_dir"] == "/path/to/headless"
        assert result_headless["user_data_cleanup"] is cleanup_headless
        assert strategy.user_data_context_provider.acquire_read_context.call_count == 2

    def test_chromium_strategy_headless_allowed_when_parity_flag_enabled(
        self, strategy: ChromiumDownloadStrategy
    ) -> None:
...
class TestTikTokDownloadStrategyFactory:
    ...
    def test_create_strategy_camoufox(self, mock_settings: MagicMock) -> None:
        ...
        with patch(
            "app.services.tiktok.download.strategies.factory.TIKTOK_DOWNLOAD_STRATEGY",
            "camoufox",
        ):
    ...
    def test_create_strategy_invalid_value(self, mock_settings: MagicMock) -> None:
        ...
        with patch(
            ...
            with pytest.raises(
                ValueError,
                match="Unsupported TikTok download strategy: invalid_strategy",
            ):
