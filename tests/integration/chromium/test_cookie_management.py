...
class TestChromiumCookieManagement:
    ...
    def test_cookie_database_corruption_recovery(self, user_data_manager):
        ...
        with user_data_manager.get_user_data_context('write') as (effective_dir, cleanup):
            ...
            cookies_db = Path(effective_dir) / "Default" / "Cookies"
        ...
        assert result.get('cookies_available') is False

        # Should be able to import new cookies after corruption
        new_cookies = {
            "format": "json",
            "cookies": [
                {
        ...
        success = user_data_manager.import_cookies(new_cookies)
        assert success is True

        # Should be able to export after recovery
        result = user_data_manager.export_cookies()
        assert len(result["cookies"]) == 1
        assert result["cookies"][0]["name"] == "recovery_cookie"
    ...
    def test_cookie_metadata_tracking(self, user_data_manager, sample_cookies):
        ...
        post_import_metadata = user_data_manager.get_metadata()
        assert post_import_metadata is not None
        assert "last_cookie_import" in post_import_metadata
        assert post_import_metadata["cookie_import_count"] == len(sample_cookies["cookies"])
        assert post_import_metadata["cookie_import_status"] == "success"

        # Export cookies
        user_data_manager.export_cookies()

        # Export shouldn't change import metadata
        final_metadata = user_data_manager.get_metadata()
        assert final_metadata["last_cookie_import"] == post_import_metadata["last_cookie_import"]
        assert final_metadata["cookie_import_count"] == post_import_metadata["cookie_import_count"]